...
while RUNNING
  ...
  # FSW reads PING_SEQ as little-endian; COSMOS 4.5 always encodes
  # APPEND_PARAMETER big-endian and ignores per-param overrides, so swap here.
  v = ping_seq & 0xFFFFFFFF
  ping_seq_be = ((v & 0xFF) << 24) | ((v & 0xFF00) << 8) | ((v & 0xFF0000) >> 8) | ((v >> 24) & 0xFF)
  cmd("GENERIC_GNSS GENERIC_GNSS_GS_PING_CC with PING_SEQ #{ping_seq_be}")
