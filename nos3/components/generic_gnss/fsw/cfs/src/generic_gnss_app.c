...
void GENERIC_GNSS_BusRxChildTask(void)
{
    ...
    while (1)
    {
        ...
        int32 avail = uart_bytes_available(&GENERIC_GNSS_AppData.GnssUart);
        if (avail <= 0)
        {
            OS_TaskDelay(GENERIC_GNSS_CFG_POLL_PERIOD_MS);
            continue;
        ...
        uint32 want = (avail > (int32)sizeof(buf)) ? sizeof(buf) : (uint32)avail;
        int32  got  = uart_read_port(&GENERIC_GNSS_AppData.GnssUart, buf, want);
        if (got <= 0)
        {
            OS_TaskDelay(GENERIC_GNSS_CFG_POLL_PERIOD_MS);
            continue;
