# Generic_gnss - NOS3 Component
This repository contains the NOS3 Generic_gnss Component.
This includes flight software (FSW), ground software (GSW), simulation, and support directories.

## Overview
This generic_gnss component is a CAN device that accepts multiple commands, including requests for telemetry and data.
The available FSW is for use in the core Flight System (cFS) while the GSW supports COSMOS.
A NOS3 simulation is available which includes both generic_gnss and 42 data providers.

An inertial measurement unit, or GNSS, is an electrical unit which uses a combination of accelerometers and gyroscopes to
...
* Generic_gnss
  - uint32, Command Counter
    * Increments for each command received
  - uint16, Data X
    * X component of generic_gnss data
  - uint16, Data Y
    * X component of generic_gnss data
  - uint16, Data Z
    * X component of generic_gnss data
...
This command will direct the GNSS to return the data on both linear and rotational acceleration in all three axes.
...
This command will contain the telemetry from the GNSS, including data on linear and rotational acceleration in all three axes.
...
## FSW
Refer to the file [fsw/platform_inc/generic_gnss_platform_cfg.h](fsw/platform_inc/generic_gnss_platform_cfg.h) for the default
...
```
<simulator>
    <name>generic_gnss_sim</name>
    <active>true</active>
    <library>libgeneric_gnss_sim.so</library>
    <hardware-model>
        <type>GENERIC_GNSS</type>
        <connections>
            <connection><type>command</type>
                <bus-name>command</bus-name>
                <node-name>generic_gnss-sim-command-node</node-name>
            </connection>
        ...
        <data-provider>
            <type>GENERIC_GNSS_PROVIDER</type>
        </data-provider>
    </hardware-model>
</simulator>
```
...
```
        <data-provider>
            <type>GENERIC_GNSS_42_PROVIDER</type>
...
# Documentation
If this generic_gnss application had an ICD and/or test procedure, they would be linked here.
