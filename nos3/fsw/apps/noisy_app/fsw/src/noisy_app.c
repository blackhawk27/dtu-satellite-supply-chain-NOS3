...
/* * * Hardcoded standard cFS Command Message IDs.
 * We use these to target every major subsystem's mailbox.
 */
...
void NOISY_APP_Main(void)
{
    ...
    CFE_SB_MsgId_t TargetMIDs[] = {
        CFE_SB_ValueToMsgId(TARGET_ES_CMD_MID),
        CFE_SB_ValueToMsgId(TARGET_EVS_CMD_MID),
        CFE_SB_ValueToMsgId(TARGET_SB_CMD_MID),
        CFE_SB_ValueToMsgId(TARGET_TIME_CMD_MID),
        CFE_SB_ValueToMsgId(TARGET_TBL_CMD_MID),
        CFE_SB_ValueToMsgId(TARGET_CI_LAB_CMD_MID),
        CFE_SB_ValueToMsgId(TARGET_TO_LAB_CMD_MID)};
    int numTargets = sizeof(TargetMIDs) / sizeof(TargetMIDs[0]);

    CFE_EVS_Register(NULL, 0, CFE_EVS_EventFilter_BINARY);
    CFE_EVS_SendEvent(1, CFE_EVS_EventType_INFORMATION, "NOISY_APP: Initiating Total Blackout (Targeting all
mailboxes)!");
    ...
    while (CFE_ES_RunLoop(&RunStatus) == true)
    {
        ...
        else if (delayCounter == 60000)
        {
            CFE_EVS_SendEvent(2, CFE_EVS_EventType_INFORMATION,
                              "NOISY_APP: 60 sekunder er gået. Initiating Total Blackout!");
