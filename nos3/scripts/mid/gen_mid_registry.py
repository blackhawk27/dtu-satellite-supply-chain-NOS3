...
# /test/ and /ut-coverage/ avoid cFE testcase headers that reuse production
# MID names with off-by-one values; /sample_defs/ skips SBN's per-cpu
# duplicates. See debug/journal.md "MID generator path filters" if any of
...
# cFE core headers in the 6.7.99 baseline declare MIDs as
#   `#define CFE_ES_CMD_MID  CFE_PLATFORM_CMD_MID_BASE + CFE_MISSION_ES_CMD_MSG  /* 0x1806 */`
# Neither LITERAL_MID_RE nor the MIDVAL/TOPICID regexes capture this base+offset
# form, but the resolved hex value is always present in a trailing block
# comment. This regex pulls it out so the cFE core MIDs land in the registry.
COMMENTED_MID_RE = re.compile(
    r"^\s*#define\s+(\w+_MID(?:_\w+)*)\s+\S.*?/\*\s*(0x[0-9A-Fa-f]+)\s*\*/",
    re.M,
)
...
def collect_bases() -> dict[tuple[str, str], int]:
    ...
    Draco builds define the masks via TOPICID_TO_MIDV macros in
    nos3_defs/{cfe_msgid_api.h,nos3_component_msgid_mapping.h}. This
    tree keeps the legacy hardcoded MID style and does not ship those
    headers, so fall back to the values implied by the existing MID
    assignments (HERITAGE_CMD=0x1800, HERITAGE_TLM=0x0800,
    ...
    if len(bases) != 4:
        bases = {
            ("HERITAGE", "CMD"): 0x1800,
            ("HERITAGE", "TLM"): 0x0800,
            ("COMPONENT", "CMD"): 0x1900,
            ("COMPONENT", "TLM"): 0x0900,
        }
    return bases
...
def resolve_mids(
    ...
    for h in header_files("msgids.h") + header_files("msgid_values.h"):
        ...
        for m in COMMENTED_MID_RE.finditer(text):
            name = m.group(1)
            if "_MIDVAL_" in name or name.startswith("CFE_PLATFORM_"):
                continue
            mids.setdefault(name, int(m.group(2), 16))
