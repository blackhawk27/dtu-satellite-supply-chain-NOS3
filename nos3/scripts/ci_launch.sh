...
if [ "$GSW" == "cosmos" ]; then
  ...
  sleep 5
  $DCALL exec -d cosmos-openc3-operator-1 bash -c "cd /cosmos/tools && ruby CmdTlmServerHeadless /cosmos/config/tools/cmd_tlm_server/cmd_tlm_server.txt > /tmp/cmd_tlm_server.log 2>&1"
