...
do
    ...
    gnome-terminal --title=$SC_NUM" - NOS3 Flight Software" -- $DFLAGS -v $BASE_DIR:$BASE_DIR --name $SC_NUM"-nos-fsw" -h nos-fsw --network=$SC_NETNAME -w $FSW_DIR --sysctl fs.mqueue.msg_max=10000 --ulimit rtprio=99 --cap-add=sys_nice --cpus=2 $DBOX $SCRIPT_DIR/fsw/fsw_respawn.sh &
    ...
    echo $SC_NUM " - CPU Monitor..."
    gnome-terminal --tab --title=$SC_NUM" - CPU Monitor" -- bash -c "$BASE_DIR/scripts/cpu_monitor.sh"
    echo ""
